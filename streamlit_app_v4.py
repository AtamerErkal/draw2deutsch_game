...
# Gelişmiş CSS stilleri (Değişiklik yapılmadı)
st.markdown("""
<style>
...
WORD_DATA = [
    ...
    {"de": "der Fisch", "en": "fish", "topic": "🐕 Hayvanlar", "difficulty": 1, "meaning": "Balık - Suda yaşayan hayvan"},
...
def mock_ai_prediction(target_word_en, canvas_data):
    ...
    predictions = []
    # Çizim ne kadar doluysa, doğru bilme ihtimali o kadar artsın (basit bir simülasyon)
    pixel_density = canvas_data[:, :, 3].sum() / (canvas_data.shape[0] * canvas_data.shape[1] * 255)
    
    correct_chance = 0.3 + (pixel_density * 3) # Yoğunluğa bağlı doğru bilme ihtimali
...
with st.sidebar:
    ...
    difficulty = st.select_slider("⭐ Zorluk Seviyesi:", options=[1, 2], format_func=lambda x: "Kolay ⭐" if x == 1 else "Orta ⭐⭐", value=1)
    ...
    if st.button("🎮 Yeni Macera Başlat!", key="new_game"):
        ...
        if len(topic_words) >= rounds:
            ...
            st.error("Bu konuda ve zorlukta yeterli kelime yok!")
...
# Nixi tanıtımı
    ...
    <div class="nixi-intro">
        <div class="nixi-character">🧠</div><h2>Merhaba! Ben Nixi! 🧠✨</h2>
        <div class="intro-text"><p>Ben sana Almanca bir kelime vereceğim ve sen onu çizeceksin! Pixi de tahmin etmeye çalışacak. Ne kadar hızlı ve net çizersen, o kadar çok puan ve rozet kazanırsın!</p></div>
    </div>
    """, unsafe_allow_html=True)
...
# Ana oyun alanı
    ...
    with col2:
        ...
        if st.session_state.last_predictions:
            st.markdown('<div class="prediction-panel"><h4 style="text-align: center;">Pixi\'nin Tahminleri</h4>', unsafe_allow_html=True)
        ...
        avg_score = st.session_state.total_score / (st.session_state.current_round + 1)
